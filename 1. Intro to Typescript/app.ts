...
function evenPosition(array: string[]): string {
    let arra: string[] = []
...
function biggerHalf(x: number[]): number[] {
    let arr: number[] = []
...
function cats(catArray: string[]): string {
    type CatType = {
    ...
    for (const el of catArray) {
        const [name, age]: string[] = el.split(' ')
...
function employees(x: string[]): string {
    const arr: string[] = []
...
function aggregateElems(x: number[]): string {
    let sum: number = 0
    let inversedSum: number = 0
    let string: string = ''
